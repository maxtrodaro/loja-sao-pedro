...
var MenuConfig = function () {
	...
	$("#reference-menu").find("h3").each(function () {
		var e = $(this);
		var lnk = e.html();
		...
		if (name != "undefined") {
			...
			if (e.next().find("li").length > 0) {
				$("#nav-menu").find(">ul").append('<li class="pai ' + name + '">' + lnk + '<div class="nivel-1"><div class="nivel-2"><h2>' + txt + '</h2><ul></ul></div><div class="destaque-menu"></div></div></li>');
			} else {
				$("#nav-menu").find(">ul").append('<li class="pai ' + name + '">' + lnk + '</li>');
			};
		}
	});

	//preenche categorias
	$("#reference-menu").find("ul").each(function () {
		var e = $(this);
		...
		if (content != "" && content != undefined) {
			$("#nav-menu").find(">ul").find(">li." + ref).find(".nivel-2").find("ul").append(content);
		};
	});
	...
	$('nav.nav-menu > ul > li:nth-child(1)').before('<li class="pai allDepartament"><p><svg xmlns="http://www.w3.org/2000/svg" id="Capa_1" x="0px" y="0px" viewBox="0 0 5.04 4.08" style="enable-background:new 0 0 5.04 4.08;" xml:space="preserve"><g><rect class="st0" width="5.04" height="0.68"/><rect y="1.7" class="st0" width="5.04" height="0.68"/><rect y="3.4" class="st0" width="5.04" height="0.68"/></g></svg>Todos os departamentos</p><ul class="allDepartaments">' + allDepartament + '</ul></li>');
	...
	$("#nav-menu").find(">ul").find(">li").each(function () {
		var e = $(this).html();
		$("footer#footer").find(".int").find("#footer-categories").append("<li>" + e + "</li>")
	});

	//Preenche o menu mobile
	var ref = $("#nav-menu").find(">ul").html();
	var a = $(".menu-mobile").find("ul");
	a.append(ref + '<li class="linkCont01"><a href="/institucional/quem-somos">Quem somos</a></li><li class="linkCont02"><a href="/institucional/nossas-lojas">Nossas Lojas</a></li><li class="linkCont03"><a href="/institucional/contato">Contato</a></li>');
		...
		if (esse.find('.boletoVitrine').length < 1) {
			...
			if (esse.find('.best-price').length > 0) {
				...
				if (percent > 0) {
					...
					esse.find('.price').append('<em class="boletoVitrine"><span><strong>R$ ' + ecomomiaBoleto2 + '</strong> no boleto</span></em>');
				};
			};
		};
	});
...
var dropdown_product_infos = function () {
	...
	$('.titles .title').on('click', function () {
		if (!$(this).hasClass('active')) {
			...
			$(this).parents('body').find('#texts div.' + _data).addClass('active');
		}
	});
};
				...
				if (avaliacaoVitrine.text() == "30") {
					...
					if (avaliacaoVitrine.text() == "20") {
						...
						if (avaliacaoVitrine.text() == "10") {
							...
							avaliacaoVitrine.removeClass().addClass('ratingStar zeroEstrelas');
						}
					};
				};
			};
...
$(function () {
	...
	if ($('body.institucional').length) {
		...
		$("ul.col.col25.eq03.conteudo-footer").find("li").each(function () {
			var e = $(this).html();
	...
	if ($('body.produto').length) {
		...
		if (mobile == "sim") {
			...
			$('#productSelect .skuList label').on('click', function () {
				$('#image-product #show .thumbs').removeAttr('class').addClass('thumbs');
				setTimeout(function () {
					slickProductMobile();
				}, 1000);
			});
